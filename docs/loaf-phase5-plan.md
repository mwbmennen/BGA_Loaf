...
| Boss character cards | `docs/card-scans/` | `angry_boss.jpg`, `happy_boss.jpg` | 2 | 600×834 JPEG, confirmed RGB — same format as the round-card scans | Provided (found during the §7 boss-pile fan layout pass, not part of the original asset inventory — see `docs/loaf-remarks.md`'s "Boss-pile fan layout" entry) |
...
| Boss cards | `img/boss-sheet.jpg` | 2 (angry, happy) | 180×251 | JPEG q90 | 2×1 | 40KB (built) |

**All 10 sheets built and verified** — `tools/build-sprite.sh` (adapted from Gelati's, §4 step
...
alpha transparency intact) — not just file-size-tested. Total `img/` file count: **10**, one
over `img/README`'s own "fewer than 10" guidance (a `10th` file, `boss-sheet.jpg`, was added
during the §7 boss-pile fan layout pass once the boss-card art was found — see
`docs/loaf-remarks.md`). **Judgment call**: accepted deliberately, not overlooked — the file
itself is trivial (40KB, 2 tiny tiles), the guidance is explicitly a "typically" soft heuristic
per `img/README`'s own wording, not a hard platform limit, and merging the boss cards into an
existing sheet (order/review, whose 6-column grid and sprite-index math are already locked in
elsewhere) would have meant reshaping an established, working lookup for two cosmetic tiles.
No further sheets are planned, so this stays the ceiling.
...
- **Done — `#boss-piles` real layout: a fixed Boss character card per pile with filed review
  cards fanned out behind it**, matching the physical rulebook's own placement diagram (boss
  card on top, review cards slid mostly underneath it, revealing only the effect band; new
  cards join at the outermost, most-hidden position, oldest stays closest to the boss and most
  visible). Two discoveries made building this, not assumed going in:
  - **`docs/card-scans/angry_boss.jpg`/`happy_boss.jpg` exist** (600×834 JPEG, confirmed RGB,
    same format as the round-card scans) but were never part of the original asset inventory
    (§3's table above didn't list them) — added to `tools/build-sprite.sh` as a new 2-tile
    `img/boss-sheet.jpg` (§4's asset manifest), the pipeline's 10th file.
  - **Filed cards must be rotated (`card.rotation: 1`, the same field/value already used for the
    pending-review-card), not left unrotated as originally implemented.** Confirmed by directly
    cropping the raw review-card scans with ImageMagick rather than guessing: the printed
    success band runs across the raw art's *top* edge and the fail band across its *bottom*
    edge (both full-width, icon→number left-to-right). Rotating 90° — exactly what `rotation: 1`
    already does for the pending-review-card — moves the success band to the card's right edge
    and the fail band to its left edge, which is why a heavily-overlapped fan growing right from
    the happy boss (covered on its boss-facing/left side) reveals green, and a fan growing left
    from the angry boss (covered on its boss-facing/right side) reveals red — no image cropping
    needed, just overlap + z-index.
  - Implementation: `bossHappyStock`/`bossAngryStock` changed from a `BgaCards.LineStock` to a
    `BgaCards.SlotStock` with `BOSS_PILE_MAX_SLOTS` (6) pre-registered slots — a `LineStock`'s
    flex layout has no way to express the per-slot overlap/z-index the fan needs. `mapCardToSlot`
    reads a `pileSlot` field this client assigns from a monotonic per-pile counter
    (`bossHappyNextSlot`/`bossAngryNextSlot`) — pile position is arrival order, not something
    derivable from the card's own data, so it can't be a pure function of the card the way
    `SlotStock` normally expects. Boss cards themselves are plain background-image divs (not
    `bga-cards` cards) — static flavor art with no game data behind it, same "plain DOM for
    board-like, non-card elements" precedent as §5's reputation track. Angry fans left, happy
    fans right (`loaf.css`'s `.loaf_boss-pile--angry`/`--happy`), so the two boss cards sit
    back-to-back in the middle of `#boss-piles` and the two fans never collide.
  - **Live-verified, three real bugs found and fixed** (`docs/loaf-remarks.md` has the full
    history): (1) per-slot positioning initially used a `.loaf_boss-pile-slot-N` class per
    `slotsIds` index, assuming `slotClasses[i]` mapped to `slotsIds[i]` — wrong; `bga-cards`'
    own source (`SlotStock.createSlot()`) adds *every* class in `slotClasses` to *every* slot, so
    every filed card carried all 6 classes and the same-specificity cascade always picked the
    last one (`slot-5`), landing every card at the pile's outermost offset regardless of its real
    slot. Fixed by targeting the library's real per-slot `data-slot-id` attribute instead
    (`[data-slot-id="N"]`), confirmed in that same source read — no `slotClasses` passed at all
    now. (2) The filed-card slots sat visibly higher than the boss card — `#boss-happy-pile`/
    `#boss-angry-pile` (the `SlotStock` element itself) was left unsized in normal flow, so it
    only grew to fit a rotated (180px-tall) card, shorter than the fan's 251px box, and its own
    50% vertical center (what the slots center on) didn't match the boss card's; fixed by
    stretching it to the fan's full `width/height: 100%`. (3) The reveal-strip width was tuned
    live through several values — the as-measured ~48px and a tried 40px both felt too cramped
    once seen with real cards, 80px read as too spread out, **60px is the final, live-confirmed
    value** for slots 1-5, wider than the raw ~18%-of-card-width measurement, chosen for
    legibility over strict physical accuracy. Slot 0 (adjacent to the boss, covered only by the
    boss card itself rather than another filed card) kept a tighter **40px**, live-confirmed
    separately — it doesn't need the same breathing room the card-on-card slots do.
  Rest of the page (`#player-tables`, `#committed-cards` spacing beyond §8's own pass) still
  uses browser block-level defaults — a full `loaf.css` pass beyond `#pending-cards`/
  `#boss-piles` remains pending (§2's own scope item).
...
- **Done** — `grep -rn "console\.log" modules/js/` and commented out (not deleted) every hit:
  `Game.js`'s constructor (`"loaf constructor"`), `setup()` (`"Starting game setup"`/`"Ending
  game setup"`), `setupNotifications()` (`"notifications subscriptions setup"` and the
  `logger: console.log` notification-debug param), and the `this.bga.states.logger =
  console.log` debug toggle — the scaffold's own comment on that last line already said "Remove
  before going to production!". The one remaining `console.warn` (`setupPlayerPanelHandCount`'s
  sibling color-resolution helper, for a genuinely unrecognized player color) was deliberately
  left alone — real error-path reporting, not development scaffolding, out of this audit's scope.
...
- [x] Every sprite sheet/board image uploads under BGA's 4MB ceiling (`du -sh img/*` checked
      before upload, not discovered after a failed sync) — verified via exact byte counts
      (`wc -c`), not `du`'s block-rounded output (which made `zoom-hand-2.jpg` read as 4.1M;
      its real size is 3,265,044 bytes, ~3.1MB, safely under).
- [x] Colors render correctly in both Chrome and Firefox specifically — the documented
      CMYK-inversion gotcha is Firefox-specific and easy to miss if only tested in one browser.
- [x] Hard-reload (`Cmd+Shift+R`) after every CSS/image sync — Studio aggressively caches CSS
      (already documented in `docs/bga-studio-reference.md`); confirmed necessary and working as
      the standing workflow throughout this session's live-verification passes.

### `bga-cards`/`bga-animations`/`bga-zoom` adoption

- [x] All three libraries import cleanly (`importEsmLib('bga-cards', '1.x')`/`'bga-animations'`/
      `'bga-zoom'`) with no console errors, and their `.d.ts` files are present for
      IDE/type-checking (§4 step 1) — implicit in everything else this session rendering/
      animating correctly, plus the explicit `console.log`-cleanup pass confirming no console
      errors across a played-through game.
- [x] `addCards()`/`removeCards()` async timing verified live across a real state transition
      ...
      were caught. Confirmed via this session's hand/commit/reveal live-push test (one player
      committing while another's browser was already open, not freshly refreshed).
- [x] `bossHappy`/`bossAngry`/`myHand` render correctly from their PHP-Deck-shaped map data —
      not silently empty from a rejected map object (§4 step 12's `array_values()` gotcha) —
      confirmed via this session's boss-pile fan testing (filed cards rendered correctly) and
      the hand/commit/reveal pass (own hand rendered correctly).
- [x] Card flip (`isCardVisible`) correctly keeps opponent hand cards and any not-yet-revealed
      committed card face-down until the right notification — a privacy regression check on top
      of the existing "opponent hands show count only" guarantee — confirmed in this session's
      hand/commit/reveal pass.
- [x] `bga-zoom`'s controls appear, actually zoom the whole board (not just part of it), and
      `localStorageZoomKey` persists the chosen level across a page reload.
- [x] Card art stays correctly positioned/aligned at zoom levels other than 100% — confirms
...
- [x] All player counts (2–6) show a token in the visually correct starting position
      (reputation 0).
- [x] A token moves to the correct new track position immediately on `reputationChanged`, no
      refresh needed.
- [x] Token color matches each player's assigned BGA color.
- [x] All 6 token sprites (real chef-hat art, not a placeholder dot) render correctly and stay
      ...
      (This whole section: **done, live-verified in PR #23** per §14 item 3 — three real bugs
      found and fixed there, see `docs/loaf-remarks.md`'s "Board & reputation-track rendering"
      entries; checkboxes just hadn't been reconciled here until now.)

### Boss piles & pending cards

- [x] The correct card art (matching `card_type` + the side that actually resolved) appears on
      the correct pile the instant a round resolves — confirmed via this session's boss-pile fan
      testing (correct color/pile, correct icon/value on each filed card).
- [x] The weighted counter (already correct as of Phase 4) still reads correctly once real card
      art replaces the plain text counter — confirmed (e.g. "Happy boss: 1 / 5") alongside the
      fan testing above.
- [x] The pending order card and pending review card both update correctly at the start of every
      new round, matching what the `roundStart`/`reviewCardRevealed` log text already says.
- [x] Hovering any round card shows the zoom-quality tooltip, correctly positioned/cropped and
...
- [x] Own hand renders as real card art via `HandStock`, one visual per value currently in hand.
- [x] Clicking a card commits it (same `actCommitCard` action as before, new visual only).
- [x] All players' played cards reveal together (simultaneously or in fast sequence) once every
      player has committed, and the revealed values match what the game log already shows —
      confirmed on a live push (another player's browser already open, not freshly refreshed),
      not just on page load.
- [x] Opponent hands still show only a count, never values, before commit — a privacy
      regression check worth deliberately re-verifying while reworking the hand component.
- [x] Mid-round page refresh reconstructs the correct state (own hand, opponents' counts,
      committed slots face-down/face-up as appropriate).

### Advanced-effect UI

- [x] Eligible cards for `discard_choice`/swap effects are visually distinguished from
      ineligible ones in the full-hand view, not just absent from a separate list.

### Cleanup

- [x] No `console.log` output in the browser console across a full played-through game.
- [x] **Revised approach, live-tested first**: a live non-English UI check turned out to be
      untestable in Studio's own tables — a private/unpublished game has no submitted
      translations at all, so switching UI language wouldn't render anything different from
      English regardless of wrapping. Substituted a static audit instead: grepped
      `modules/js/Game.js` for user-facing template-literal text not wrapped in `_()`, which
      caught a real gap Phase 5 itself introduced — `#pending-cards`' "Next order card"/"Current
      review card" and `#boss-piles`' "Angry boss"/"Happy boss" labels (§7's original pass) were
      all bare literals — now wrapped. Everything else (player-panel reputation/hand-count,
      "FIRED") was already correctly wrapped.

### Close it out

- [x] Update `docs/loaf-remarks.md` with a "Phase 5 live verification" entry, same convention
...
3. **Done, live-verified** — Board & reputation track (§5): `img/board.png` as the track
   background, real chef-hat token art (`img/tokens.png`) positioned per player via measured
   pixel geometry rather than the old per-player text line, plus a reputation readout on BGA's
   own standard player panel. Three live-verification bugs found and fixed on Studio (a
   mismeasured column divider, the board's asymmetric vertical layout between halves, and a
   stacked pair of silent bugs -- an uncast DB string plus an unmatched player color -- that
   made one token fully invisible after a refresh); see `docs/loaf-remarks.md`'s "Board &
   reputation-track rendering" entries for the full judgment-call and bug-fix history. Merged
   in PR #23.
4. **Done, not yet live-verified** — Hand/commit/reveal (§8): a real `HandStock` replaces the
   status-bar action-button list, one face-down "committed" `SlotStock` per player, flipped
   face-up via `CardManager.updateCardInformations()` on `cardPlayedRevealed`. A missing
   `(int)` cast on `myHand` was caught and fixed proactively (not via live debugging, unlike
   every prior instance of this bug class this session) before it ever shipped broken; see
   `docs/loaf-remarks.md`'s "Phase 5 §8: hand, commit, and reveal" entry for the full judgment
   calls (committed-card identity scheme, uniform hidden-until-reveal treatment including the
